...
Sequence("intro","instructions", "transition", "test",SendResults(),"bye")
...
,
	...
	newVideo("eventTest", "https:" + row.URL)
		...
		.disable(0.01)
,
    newText("testcontext","<p style=font-size:30px;> Exercise: " +row.Context+"</p>").unfold(3000)
,
	newCanvas("testcanv",1366, 550)
	    ...
	    .print()
,
	newTimer("wait", row.ContextDuration)                  
		.start()       
		.wait()
,     
...
,
	newCanvas("TestQ",1600, 566)
		...
		.add("center at 50%","center at 50%", newText("TQ1","<p style=font-size:29px;>Her time is up. Did Ebony do the exercise?</p>"))
...
,
    ...
    .log("CorrectResp", row.CorrectResp)
    .log("Context", row.Context)
    .log("ContextReminder", row.ContextReminder)
    .log("Telicity", row.Telicity)
    .log("Video", row.Video)
    .log("URLCode", row.URLCode))  
...
newTrial( "bye" ,
    newText("<p style=font-size:18px;>Your results have been saved, but you need to validate your participation below. This is a necessary step to approve your submission!</p>" +
			"<p style=font-size:18px;><a href='https://app.prolific.co/submissions/complete?cc=6BECC3B3'>Click here to confirm my submission on Prolific</a>.</p>")
		.center()
        .print()
		.wait()
).setOption("hideProgressBar",true)
